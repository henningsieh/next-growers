...
export default function ReportCard({ image, country, badges, report }: ReportCardProps) {
  ...
  const { mutate: deleteMutation } = api.reports.deleteReport.useMutation({
      ...
      onError: (err, newTodo, context) => {
        ...
        if (!context) return
        trpc.reports.getAllReports.setData(undefined, () => context.previousReports)
      },
...
{/*           <Text mt="md" className={classes.label} c="dimmed">
        ...
        <Group mt="xs">
          <Button fullWidth variant='filled' color='red' radius="sm" style={{ flex: 0 }}
            className='border-red-600 border-1' 
            onClick={() => { deleteMutation(id) }}>
            Delete <IconAlertTriangleFilled className='ml-2' height={18} stroke={1.5} />
          </Button>
          <Button className='border-orange-600' variant='default' color='orange.8' radius="sm" style={{ flex: 1 }}>
            Show details
          </Button>{/* 
